...
impl MysqlFamily {
    ...
    /// Stream `mysqldump` straight into `mysql`, with whatever extra flags the caller
    /// needs on the dump.
    ///
    /// **One OS pipe between two client processes** — the same thing `mysqldump | mysql`
    /// is at a shell prompt: the script never becomes a file and never passes through
    /// sloop's memory either.
    fn stream_a_dump(
        &self,
        source: &Target<'_>,
        destination: &Target<'_>,
        ...
        // alongside a list of tables makes `mysqldump` refuse outright, and a scoped dump is
        // not the thing that should be carrying them anyway.
        if tables.is_empty() {
        ...
        // The database first and the tables after it: that is `mysqldump`'s own word order,
        // and it is positional rather than a flag.
        command.arg(source.database).args(tables);
...
impl Adapter for MysqlFamily {
    ...
    fn copy_schema_into(
        ...
        // `--no-data` is the whole difference from `copy_into`: the tables, the views, the
        // routines and the triggers, and not one row.
        //
        // Named tables go after the database, which is `mysqldump`'s own way of scoping a
        // dump — and it takes them literally rather than as patterns.
        let named: Vec<String> = only.iter().map(|table| table.name.clone()).collect();
        self.stream_a_dump(source, destination, &["--no-data"], &named)
    }
