...
The password of the role being *created* is generated by sloop — 28 characters, letters
and digits, so it pastes into any config file without escaping — filed where this machine
keeps secrets, and printed once so you can put it in your application. Pass
--role-password-stdin to choose it yourself instead.
