 ...
 * ## How these were made
 *
 * A throwaway PostgreSQL 17.9 cluster was built on port 5440 with `initdb`,
 * seeded with two tables — 1,284 customers and 9,317 orders — and every command
 * below was run against it, in order, on 2026-09-19. The cluster was torn down
 * afterwards. `sloop setup` was run for real on the same machine: it found the
 * PostgreSQL 17 already there, left it alone, downloaded PostgreSQL 18.6,
 * checked it, and built sloop's own cluster on 5433.
 *
 * The transcripts come from `sloop --log-file`, which is the CLI writing down
 * what it printed, with the colour dropped and anything that reads like a
 ...
 * everywhere; only the store they name changes. Those are the same named
 * substitution `A6` makes — `~/.sloop` spelled the way each platform spells it.
 *
 * **`setup` genuinely differs, and only the Windows run exists.** On Windows it
 * downloads the official binaries; on Linux and macOS it offers the machine's
 * own package manager. So the Windows block is the real run, and the Linux and
 * macOS blocks are **cut short at the question with a marked `…`** — they hold
 * the command, the offer and the question, every line of which comes out of
 * `Plan::describe()`, `Plan::question()` and `server_plan()` in
 * `cli/src/tools/acquire.rs`, and nothing after it. What follows the question on
 * those platforms was not observed and is therefore not drawn: whether sloop
 * uses the PostgreSQL the package manager just started on 5432 or builds its own
 * on 5433 depends on that machine, and a transcript that guessed would be the
...
export class GettingStarted {
  ...
  private readonly backupPath = computed(() => {
    const parts = ['backups', 'postgres', 'shop', '20260919T120946Z'];
  ...
  protected readonly register: readonly TerminalLine[] = [
    {
      kind: 'prompt',
      text: 'sloop db add shop --global --url postgres://app@127.0.0.1:5440/shop',
    },
    { text: 'registered shop in the global registry (--global), password from the OS keyring' },
    { kind: 'dim', text: '  postgres://app@127.0.0.1:5440/shop' },
  ];
  ...
  protected readonly test: readonly TerminalLine[] = [
    ...
    { text: ' ' },
    {
      ...
      tag: 'shop',
      text: '',
      note: '  postgres://app@127.0.0.1:5440/shop  global',
    },
  ...
  protected readonly firstBackup = computed<readonly TerminalLine[]>(() => [
    ...
    { kind: 'name', tag: 'shop', text: '', note: '  postgres://app@127.0.0.1:5440/shop' },
    { kind: 'dim', text: '  postgres 17.9, 2 tables, 10601 rows' },
    { kind: 'dim', text: `  backed up to ${this.backupPath()}` },
    { kind: 'dim', text: '  88.8 KiB in 0.3s, sha256 a2009c990035' },
    { kind: 'dim', text: '  taken 2026-09-19 18:09:46 +06:00 (2026-09-19T12:09:46Z)' },
  ]);
  ...
  protected readonly listBackups: readonly TerminalLine[] = [
    ...
    { kind: 'name', tag: 'shop', text: '', note: '  postgres · 1 backup, 88.8 KiB' },
    { kind: 'dim', text: '  2026-09-19 18:09:46 +06:00  88.8 KiB, encrypted · 10601 rows' },
    { text: ' ' },
    { kind: 'dim', text: '1 backup, 88.8 KiB · sizes checked, --check hashes them' },
    { text: ' ' },
    { kind: 'prompt', text: 'sloop backups list --global --check' },
    { kind: 'name', tag: 'shop', text: '', note: '  postgres · 1 backup, 88.8 KiB' },
    { kind: 'dim', text: '  2026-09-19 18:09:46 +06:00  88.8 KiB, encrypted · 10601 rows' },
    { text: ' ' },
    { kind: 'dim', text: '1 backup, 88.8 KiB · every dump hashed' },
  ];
  ...
  protected readonly restore = computed<readonly TerminalLine[]>(() => [
    ...
    {
      kind: 'dim',
      text: '  88.8 KiB of postgres 17.9, encrypted, 10601 rows, sha256 a2009c990035',
    },
  ...
  protected readonly manifest: readonly TerminalLine[] = [
    ...
    { kind: 'dim', text: '  "source": "postgres://app@127.0.0.1:5440/shop",' },
    { kind: 'dim', text: '  "server": { "version": "17.9", "tls": false },' },
    { kind: 'dim', text: '  "taken": {' },
    { kind: 'dim', text: '    "utc": "2026-09-19T12:09:46Z",' },
    { kind: 'dim', text: '    "local": "2026-09-19T18:09:46+06:00",' },
    { kind: 'dim', text: '    "offset": "+06:00", "offset_seconds": 21600,' },
    { kind: 'dim', text: '    "unix": 1789819786' },
    { kind: 'dim', text: '  },' },
    { kind: 'dim', text: '  "took_seconds": 0.505,' },
    { kind: 'dim', text: '  "dump_seconds": 0.334,' },
    { kind: 'dim', text: '  "dump": {' },
    { kind: 'dim', text: '    "file": "dump.age",' },
    { kind: 'dim', text: '    "bytes": 90907,' },
    { kind: 'dim', text: '    "sha256": "a2009c9900354fd04a055772b11ca2c1d363ac7f...",' },
...
/** The real Windows run, from `sloop setup --log-file`. */
const WINDOWS_SETUP: readonly TerminalLine[] = [
  ...
  {
    kind: 'dim',
    text: '  Downloading https://get.enterprisedb.com/postgresql/postgresql-18.6-3-windows-x64-binaries.zip',
  },
  { kind: 'ok', tag: '  Verified SHA-256 matches', text: '' },
  { kind: 'dim', text: '  Installed into C:\\Users\\you\\.sloop\\postgres\\18' },
  { kind: 'dim', text: '  Creating C:\\Users\\you\\.sloop\\postgres\\data' },
  { kind: 'dim', text: '  Started PostgreSQL 18.6 on 5433' },
  { kind: 'dim', text: '  Created sloop_db_admin' },
  { kind: 'dim', text: '  Created sloop_database' },
  { kind: 'head', tag: 'Prepared:', text: " sloop's own cluster" },
  {
    kind: 'dim',
    text: '  postgres://postgres@127.0.0.1:5433/postgres  binaries at C:\\Users\\you\\.sloop\\postgres\\18\\bin',
  },
  { kind: 'dim', text: '  cluster at C:\\Users\\you\\.sloop\\postgres\\data' },
  {
    kind: 'head',
    tag: 'State:',
  ...
  {
    kind: 'dim',
    text: '  owned by sloop_db_admin, whose password is kept where this machine keeps secrets',
  },
  {
    kind: 'dim',
    text: '  Schema version 5 — applied ledger, registry, history, service, vault from version 0',
  },
];
